...
Public Class FrmContraseña
    ...
    Private Sub FrmContraseña_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        With c_Neg_Usuario.get_Usuario_Datos(" and c_anula_reg=0 and c_nom_pc='" & UCase(My.Computer.Name) & "'", "DAT")
            If .Rows.Count > 0 Then
                TxtUser.Text = .Rows(0)("c_codi_usua").ToString
                TxtClave.Focus()
            End If
        End With
    End Sub
    ...
    Private Sub OK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OK.Click
        With c_Neg_Usuario.get_Usuario_Datos(" and c_anula_reg=0 and c_codi_usua='" & TxtUser.Text & "' and c_clave_usua='" & TxtClave.Text & "'", "DAT")
            If .Rows.Count > 0 Then
                FrmMenu.Show()
                Dim RUTA_CONCAR As String = ""
                FrmMenu.lblusuario.Text = .Rows(0)("c_codi_usua").ToString
                If Val(.Rows(0)("c_usua_admin").ToString) = 1 Then FrmMenu.ChkAdmin.Checked = True
                If Val(.Rows(0)("c_usua_transforma").ToString) = 1 Then FrmMenu.ChkTransforma.Checked = True

                Me.Hide()
                With c_Neg_MnEmpresa.get_EmpConcar_Datos(" AND E.c_codi_emp='FA' ", "DAT")
                    If .Rows.Count > 0 Then
                        FrmMenu.TxtConcar_Ruta.Text = .Rows(0)("c_ruta_concar").ToString
                        FrmMenu.TxtEmpresa.Text = .Rows(0)("c_raz_emp").ToString
                        FrmMenu.TxtRuc_Emp.Text = .Rows(0)("c_ruc_emp").ToString

                        FrmMenu.TxtRetencion.Text = Format(Val(.Rows(0)("c_porc_reten").ToString), "##")
                        FrmMenu.TxtConcar_Anexo.Text = .Rows(0)("c_concar_anexo").ToString
                        FrmMenu.TxtConcar_AnexoDet.Text = .Rows(0)("c_concar_anexodet").ToString
                        FrmMenu.TxtConcar_Plan.Text = .Rows(0)("c_concar_plan").ToString
                        FrmMenu.TxtConcar_Areas.Text = .Rows(0)("c_concar_areas").ToString
                    End If
                End With
                ...
                Call Cargar_Archivo() : Call Cargar_Datos_BD() : FrmMenu.TxtCod_Emp.Text = "FA"
